...
// packages/threat-prevention/dist/index.js
var index_exports = {};
...
server.tool(
  ...
  "Show a threat rule object by UID, name, or rule number (at least one of them). Layer is also mandatory!",
  {
    ...
    details_level: import_zod.z.enum(["uid", "standard", "full"]).optional()
  },
  async ({ uid, name, rule_number, layer, details_level }, extra) => {
    const body = {};
    if (uid) body.uid = uid;
    if (name) body.name = name;
    if (rule_number !== void 0) body["rule-number"] = rule_number;
...
server.tool(
  ...
  "Show the entire Threat Prevention Rules layer, with advanced filtering and options. Either name or UID must be provided, but not both.",
  {
    ...
    uid: import_zod.z.string().optional(),
    filter: import_zod.z.string().optional(),
    filter_settings: import_zod.z.object({
      search_mode: import_zod.z.enum(["general", "packet"]).optional(),
      packet_search_settings: import_zod.z.object({
        expand_group_members: import_zod.z.boolean().optional(),
        expand_group_with_exclusion_members: import_zod.z.boolean().optional(),
        match_on_any: import_zod.z.boolean().optional(),
        match_on_group_with_exclusion: import_zod.z.boolean().optional(),
        match_on_negate: import_zod.z.boolean().optional()
      }).optional()
    }).optional(),
    ...
    use_object_dictionary: import_zod.z.boolean().optional(),
    dereference_group_members: import_zod.z.boolean().optional(),
    show_membership: import_zod.z.boolean().optional(),
    details_level: import_zod.z.enum(["uid", "standard", "full"]).optional()
  },
  async ({ name, uid, filter, filter_settings, limit, offset, order, package: pkg2, use_object_dictionary, dereference_group_members, show_membership, details_level }, extra) => {
    const body = {};
    if (name) body.name = name;
    if (uid) body.uid = uid;
    ...
    if (filter_settings) {
      ...
      if (filter_settings.packet_search_settings) {
        ...
        if (p.expand_group_members !== void 0) body["filter-settings"]["packet-search-settings"]["expand-group-members"] = p.expand_group_members;
        if (p.expand_group_with_exclusion_members !== void 0) body["filter-settings"]["packet-search-settings"]["expand-group-with-exclusion-members"] = p.expand_group_with_exclusion_members;
        if (p.match_on_any !== void 0) body["filter-settings"]["packet-search-settings"]["match-on-any"] = p.match_on_any;
        if (p.match_on_group_with_exclusion !== void 0) body["filter-settings"]["packet-search-settings"]["match-on-group-with-exclusion"] = p.match_on_group_with_exclusion;
        if (p.match_on_negate !== void 0) body["filter-settings"]["packet-search-settings"]["match-on-negate"] = p.match_on_negate;
      }
    }
    if (limit !== void 0) body.limit = limit;
    if (offset !== void 0) body.offset = offset;
    if (order) body.order = order;
    if (pkg2) body.package = pkg2;
    if (use_object_dictionary !== void 0) body["use-object-dictionary"] = use_object_dictionary;
    if (dereference_group_members !== void 0) body["dereference-group-members"] = dereference_group_members;
    if (show_membership !== void 0) body["show-membership"] = show_membership;
    if (details_level) body["details-level"] = details_level;
    const params = { body };
...
server.tool(
  ...
  "Show a threat protection object by name or UID (one of which must be provided), with optional details and calculated fields.",
  {
    name: import_zod.z.string().optional(),
    uid: import_zod.z.string().optional(),
    show_capture_packets_and_track: import_zod.z.boolean().optional(),
    show_ips_additional_properties: import_zod.z.boolean().optional(),
    show_profiles: import_zod.z.boolean().optional(),
    details_level: import_zod.z.enum(["uid", "standard", "full"]).optional()
  },
  async ({ name, uid, show_capture_packets_and_track, show_ips_additional_properties, show_profiles, details_level }, extra) => {
    ...
    if (name) body.name = name;
    if (uid) body.uid = uid;
    if (show_capture_packets_and_track !== void 0) body["show-capture-packets-and-track"] = show_capture_packets_and_track;
    if (show_ips_additional_properties !== void 0) body["show-ips-additional-properties"] = show_ips_additional_properties;
    if (show_profiles !== void 0) body["show-profiles"] = show_profiles;
    if (details_level) body["details-level"] = details_level;
    const params = { body };
...
server.tool(
  ...
  "Show the entire Threat Exceptions layer for a given threat rule, with advanced filtering and options. Name and UID refer to the layer, one of them must be provided, but not both. In addition one of rule_uid, rule_name or rule_number must be provided to specify the rule for which exceptions are shown.",
  {
    name: import_zod.z.string().optional(),
    uid: import_zod.z.string().optional(),
    rule_uid: import_zod.z.string().optional(),
  ...
  async ({ name, uid, rule_uid, rule_name, rule_number, filter, filter_settings, limit, offset, order, package: pkg2, use_object_dictionary, dereference_group_members, show_membership, details_level }, extra) => {
    const body = {};
    if (name) body.name = name;
    if (uid) body.uid = uid;
...
server.tool(
  ...
  {
    cve: import_zod.z.string().describe("The CVE to check for protection"),
    gateway: import_zod.z.string().optional().describe("The gateway to check for protection. Leave empty to check all gateways.")
  },
  async ({ cve, gateway }, extra) => {
    ...
    const gateways = Array.isArray(gatewaysResp.objects) ? gatewaysResp.objects.filter((gw) => !gateway || gw.name === gateway) : [];
    const rulebases = /* @__PURE__ */ new Set();
    ...
    return {
      content: [
        { type: "text", text: JSON.stringify({ protections: protectionsResp, gateways, rulebases: Array.from(rulebases), profiles }, null, 2) }
      ]
    };
  }
);
