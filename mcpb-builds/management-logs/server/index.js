...
// packages/management-logs/dist/index.js
var index_exports = {};
...
server.tool(
  ...
  {
    "ignore-warnings": import_zod.z.boolean().optional(),
    // New query parameters
    "new-query": import_zod.z.object({
      filter: import_zod.z.string().optional(),
      "time-frame": import_zod.z.enum(["last-7-days", "last-hour", "today", "last-24-hours", "yesterday", "this-week", "this-month", "last-30-days", "all-time", "custom"]).optional(),
      "custom-start": import_zod.z.string().optional(),
      "custom-end": import_zod.z.string().optional(),
      "max-logs-per-request": import_zod.z.number().min(1).max(100).optional(),
      top: import_zod.z.object({
        count: import_zod.z.number().min(1).max(50),
        field: import_zod.z.enum(["sources", "destinations", "services", "actions", "blades", "origins", "users", "applications"])
      }).optional(),
      type: import_zod.z.enum(["logs", "audit"]).optional(),
      "log-servers": import_zod.z.array(import_zod.z.string()).optional()
    }).optional(),
    // Alternative: query by ID for pagination
    "query-id": import_zod.z.string().optional()
  },
  async (args, extra) => {
    const ignoreWarnings = typeof args["ignore-warnings"] === "boolean" ? args["ignore-warnings"] : void 0;
    const newQuery = args["new-query"];
    const queryId = typeof args["query-id"] === "string" ? args["query-id"] : void 0;
    const params = {};
