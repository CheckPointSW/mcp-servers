...
// packages/spark-management/dist/index.js
var index_exports = {};
...
function createServerModule(server2, Settings3, pkg2, apiManagerClass) {
  const settingsManager = new SettingsManager(Settings3);
...
var APIClientBase = class {
  ...
  */
   ...
   */
  static createWithSid(originalClient, sid) {
  ...
  async login() {
    ...
    if (loginResp.response.uid) {
      await this.detectMDS(loginResp.response.uid);
    }
   ...
   */
  async detectMDS(sessionUid, handleSelfSigned = false) {
    try {
      let httpsAgent;
      if (handleSelfSigned) {
        httpsAgent = new import_https.default.Agent({ rejectUnauthorized: false });
      }
      const sessionResp = await this.makeRequest(this.getHost(), "POST", "show-session", { uid: sessionUid }, this.getHeaders(), null, httpsAgent);
      if (sessionResp.status === 200 && sessionResp.response?.domain?.["domain-type"] === "mds") {
        this.isMDS = true;
      }
    } catch (error) {
...
var BearerTokenAPIClient = class extends APIClientBase {
...
var OnPremAPIClient = class extends APIClientBase {
  ...
  */
  async login() {
    ...
    if (loginResp.response.uid) {
      await this.detectMDS(loginResp.response.uid, true);
    }
    return loginResp.response.sid;
  }
};
...
var APIManagerBase = class {
  constructor(client) {
    ...
    this._debug = false;
    this.domainClients = /* @__PURE__ */ new Map();
    this.gatewayDomainMap = /* @__PURE__ */ new Map();
    this.domains = null;
  }
  /**
   ...
   * Call an API endpoint
   */
  async callApi(method, uri, data, domain) {
    const sanitizedData = sanitizeData(data);
    const apiClient = domain ? await this.getDomainApiClientByDomain(domain) : this.client;
    const clientResponse = await apiClient.callApi(method, uri, sanitizedData, void 0);
   ...
   */
  async getDomainApiClient(gatewayName) {
   ...
   */
  async getDomainApiClientByDomain(domainName) {
    const isMDS = await this.client.isMDSEnvironment();
    if (!isMDS) {
      return this.client;
    }
  ...
  async getGatewayInfo(gatewayName) {
    ...
    if (response.objects) {
      return response.objects.find((obj) => obj.name === gatewayName);
    }
    return null;
  }
  /**
   ...
   */
  async loginToDomain(domainName) {
  ...
  async runScript(targetGateway, scriptName, script) {
    ...
    const clientResponse = await apiClient.callApi("post", "run-script", payload, void 0);
   ...
   */
  async getTaskResult(gatewayName, taskId, maxRetries = 5) {
...
var APIManagerForBearerToken = class extends APIManagerBase {
  static create(args) {
    if (!args.bearerToken) {
    ...
    const bearerClient = new BearerTokenAPIClient(args.bearerToken, smpApiUrl);
...
var SMPAPIManager = class _SMPAPIManager {
  constructor(settings) {
    this.settings = settings;
   ...
   */
  static create(settings) {
    return new _SMPAPIManager(settings);
  }
  /**
...
var Settings2 = class _Settings2 extends Settings {
  constructor({
...
server.tool(
  ...
  {
    gatewayName: import_zod.z.string().describe("Name of the gateway to show")
  },
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error showing gateway: ${error.message}`
          }
        ]
      };
    }
...
server.tool(
  ...
  "Show gateway fields for a specific gateway object from Spark Management",
  {
    gateway: import_zod.z.object({
      name: import_zod.z.string().describe("Name of the gateway"),
      fields: import_zod.z.array(import_zod.z.string()).describe("Array of required fields, each field consists of a path, delimited by '.', to an existing topic at the gateway")
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error showing gateway fields: ${error.message}`
          }
        ]
      };
    }
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error showing gateway attributes: ${error.message}`
          }
        ]
      };
    }
...
server.tool(
  ...
  {
    gatewayName: import_zod.z.string().describe("Name of the gateway"),
    adminName: import_zod.z.string().describe("Name of the administrator to retrieve")
  },
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting gateway administrator: ${error.message}`
          }
        ]
      };
    }
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting gateway administrators: ${error.message}`
          }
        ]
      };
    }
...
server.tool(
  ...
  {
    gatewayName: import_zod.z.string().describe("Name of the gateway"),
    username: import_zod.z.string().describe("Name of the user to retrieve")
  },
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting gateway user: ${error.message}`
          }
        ]
      };
    }
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting gateway users: ${error.message}`
          }
        ]
      };
    }
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting gateway geolocation: ${error.message}`
          }
        ]
      };
    }
...
server.tool(
  ...
  {
    startIndex: import_zod.z.number().optional().describe("Page offset from which the fetch starts"),
    maxItems: import_zod.z.number().optional().describe("Page size"),
    filter: import_zod.z.object({
      freeTextSearch: import_zod.z.object({
        value: import_zod.z.string().describe("Filter by free text. Will return gateways with names matching this filter (as substring)")
      }).optional(),
      connected: import_zod.z.object({
        value: import_zod.z.boolean().describe("Connected to the Spark Management filter")
      }).optional(),
      disconnected: import_zod.z.object({
        value: import_zod.z.boolean().describe("Disconnected from the Spark Management filter")
      }).optional(),
      disabled: import_zod.z.object({
        value: import_zod.z.boolean().describe("Disabled gateway filter")
      }).optional(),
      macAddressList: import_zod.z.object({
        value: import_zod.z.array(import_zod.z.string()).describe("List of mac addresses")
      }).optional(),
      vendor: import_zod.z.object({
        value: import_zod.z.enum(["Small Office Appliance", "Small Office HA", "Small Office HA Member"]).describe("Vendor filter")
      }).optional(),
      gatewayType: import_zod.z.object({
        value: import_zod.z.array(import_zod.z.enum([
          "600 Appliance",
          "1200R Appliance",
          "730 Appliance",
          "750 Appliance",
      ...
      plan: import_zod.z.object({
        value: import_zod.z.string().describe("Plan name")
      }).optional(),
      reportedFirmware: import_zod.z.object({
        value: import_zod.z.array(import_zod.z.string()).describe("Firmware filter")
      }).optional(),
      lastConnection: import_zod.z.object({
        value: import_zod.z.object({
          from: import_zod.z.number().describe("Start time"),
          to: import_zod.z.number().describe("End time")
        }).describe("Time filter value")
      }).optional(),
      creationDate: import_zod.z.object({
        value: import_zod.z.object({
          from: import_zod.z.number().describe("Start time"),
          to: import_zod.z.number().describe("End time")
        }).describe("Time filter value")
      }).optional(),
      modificationDate: import_zod.z.object({
        value: import_zod.z.object({
          from: import_zod.z.number().describe("Start time"),
          to: import_zod.z.number().describe("End time")
        }).describe("Time filter value")
  ...
  async ({ startIndex, maxItems, filter }, extra) => {
    try {
      const data = {};
      if (startIndex !== void 0) data.startIndex = startIndex;
      if (maxItems !== void 0) data.maxItems = maxItems;
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting gateway list: ${error.message}`
          }
        ]
      };
    }
...
server.tool(
  ...
  {
    plan: import_zod.z.object({
      name: import_zod.z.string().describe("Name of the plan")
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error showing plan: ${error.message}`
          }
        ]
      };
    }
...
server.tool(
  ...
  "Get a list of plans from Spark Management",
  {
    ...
    filter: import_zod.z.object({
      freeTextSearch: import_zod.z.object({
        value: import_zod.z.string().describe("Filter by free text. Will return plans with names matching this filter (as substring)")
      }).optional()
    }).optional().describe("Search filter")
  },
  async ({ startIndex, maxItems, filter }, extra) => {
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting plan list: ${error.message}`
          }
        ]
      };
    }
...
server.tool(
  ...
  "Show plan fields for a specific plan object from Spark Management",
  {
    plan: import_zod.z.object({
      name: import_zod.z.string().describe("Name of the plan"),
      fields: import_zod.z.array(import_zod.z.string()).describe("Array of required fields, each field consists of a path, delimited by '.', to an existing topic at the plan")
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error showing plan fields: ${error.message}`
          }
        ]
      };
    }
...
server.tool(
  ...
  {
    planName: import_zod.z.string().describe("Name of the plan"),
    adminName: import_zod.z.string().describe("Name of the administrator to retrieve")
  },
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting plan administrator: ${error.message}`
          }
        ]
      };
    }
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting plan administrators: ${error.message}`
          }
        ]
      };
    }
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting plan user: ${error.message}`
          }
        ]
      };
    }
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting plan users: ${error.message}`
          }
        ]
      };
    }
...
server.tool(
  ...
  "Show an existing user object from Spark Management",
  {
    user: import_zod.z.object({
      uid: import_zod.z.string().describe("The unique ID of the user")
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error showing user: ${error.message}`
          }
        ]
      };
    }
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting portal settings: ${error.message}`
          }
        ]
      };
    }
