...
// packages/threat-emulation/dist/index.js
var index_exports = {};
...
function createServerModule(server2, Settings, pkg2, apiManagerClass) {
...
init_ui_dialog();
var ThreatEmulationSettings = class _ThreatEmulationSettings {
  constructor({
  ...
  static fromArgs(options) {
    return new _ThreatEmulationSettings({
...
var ThreatEmulationClient = class {
  constructor(settings) {
    this.BASE_URL = "https://te-api.checkpoint.com/tecloud/api/v1/file";
    this.settings = settings;
  }
  /**
   * Upload file for threat analysis
   */
  async uploadFile(filePath, fileName, features = ["te", "av"], hashes, reports) {
    try {
      ...
      if (features.includes("te")) {
        request.te = {
          reports: reports || ["xml"]
          // Changed: Default to XML only
        };
      }
      formData.append("request", JSON.stringify({ request }));
      formData.append("file", (0, import_fs.createReadStream)(filePath));
      const response = await import_axios.default.post(`${this.BASE_URL}/upload`, formData, {
        headers: {
   ...
   */
  async queryFile(features = ["te", "av"], hashes, images, fileName, reports) {
    try {
      const makeQueryRequest = async (requestReports) => {
        const request = {
          features
        };
        if (fileName) {
        ...
        return await import_axios.default.post(`${this.BASE_URL}/query`, {
          request
        }, {
          headers: {
            "Authorization": this.settings.apiKey,
  ...
  async downloadReport(reportId) {
    try {
      const response = await import_axios.default.get(`${this.BASE_URL}/download`, {
        params: { id: reportId },
        headers: {
          "Authorization": this.settings.apiKey
        }
  ...
  async getQuota() {
    try {
      const response = await import_axios.default.post(`${this.BASE_URL}/quota`, {}, {
        headers: {
          "Authorization": this.settings.apiKey,
          "Content-Type": "application/json"
        }
...
function calculateMD5(filePath) {
  const fileBuffer = (0, import_fs3.readFileSync)(filePath);
  return (0, import_crypto2.createHash)("md5").update(fileBuffer).digest("hex");
}
var pkg = JSON.parse(
  (0, import_fs4.readFileSync)((0, import_path.join)((0, import_path.dirname)((0, import_url2.fileURLToPath)(url.pathToFileURL(__filename).toString())), "../package.json"), "utf-8")
);
...
server.tool(
  ...
  {
    file_path: external_exports.string().describe("Path to the file to upload (use this for local files)"),
    file_name: external_exports.string().optional().describe("Name of the file (optional, extracted from path if not provided)"),
    features: external_exports.array(external_exports.enum(["te", "av"])).optional().default(["te", "av"]).describe("Features to use for analysis"),
    reports: external_exports.array(external_exports.enum(["xml", "summary"])).optional().default(["xml"]).describe("Report types to generate (default: xml only)"),
    md5: external_exports.string().optional().describe("MD5 hash of the file for validation"),
    sha1: external_exports.string().optional().describe("SHA-1 hash of the file for validation"),
    sha256: external_exports.string().optional().describe("SHA-256 hash of the file for validation")
  },
  async (args, extra) => {
    try {
      const filePath = args.file_path;
      const features = args.features || ["te", "av"];
      const reports = args.reports || ["xml"];
      if (!filePath) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "file_path is required" }, null, 2) }] };
      }
      try {
        (0, import_fs4.statSync)(filePath);
      } catch {
        return { content: [{ type: "text", text: JSON.stringify({ error: "File not found" }, null, 2) }] };
      }
      const actualFilePath = filePath;
      const actualFileName = args.file_name || (0, import_path.basename)(actualFilePath);
      const hashes = {};
...
server.tool(
  ...
  {
    file_name: external_exports.string().optional().describe("Name of the file (optional - helpful for context)"),
    file_path: external_exports.string().optional().describe("Path to file if available - used to calculate MD5 if not provided"),
    features: external_exports.array(external_exports.enum(["te", "av"])).optional().default(["te", "av"]).describe("Features to query for"),
    md5: external_exports.string().optional().describe("MD5 hash of the file (required for Anti-Virus analysis)"),
    sha1: external_exports.string().optional().describe("SHA-1 hash of the file"),
    sha256: external_exports.string().optional().describe("SHA-256 hash of the file"),
    reports: external_exports.array(external_exports.enum(["xml", "summary"])).optional().default(["xml"]).describe("Report types to query (default: xml only)"),
    images: external_exports.array(external_exports.object({
      id: external_exports.string().describe('Image ID (e.g., "5e5de275-a103-4f67-b55b-47532918fa59")'),
      revision: external_exports.number().describe("Image revision (typically 1)")
...
server.tool(
  ...
  {
    file_path: external_exports.string().describe("Path to the file to scan"),
    file_name: external_exports.string().optional().describe("Name of the file (optional for file_path)"),
    wait_timeout: external_exports.number().optional().default(30).describe("Maximum seconds to wait for results (default: 30)"),
    reports: external_exports.array(external_exports.enum(["xml", "summary", "tar"])).optional().default(["xml"]).describe("Report types to generate (default: xml only)")
  },
  async (args, extra) => {
    try {
      const filePath = args.file_path;
      const timeout = args.wait_timeout || 30;
      const reports = args.reports || ["xml"];
      if (!filePath) {
        return { content: [{ type: "text", text: JSON.stringify({ error: "file_path is required" }, null, 2) }] };
      }
      ...
      try {
        const existingResult = await client.queryFile(["te", "av"], { md5 }, void 0, actualFileName, reports);
      ...
      while ((Date.now() - startTime) / 1e3 < timeout) {
        await new Promise((resolve) => setTimeout(resolve, 5e3));
        const result = await client.queryFile(["te", "av"], { md5 }, void 0, actualFileName, reports);
...
server.tool(
  ...
  {
    report_id: external_exports.string().describe("XML report ID to download. Must be UUID from report.xml_report field, NOT an image ID. Format: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx")
  },
  async (args, extra) => {
    try {
