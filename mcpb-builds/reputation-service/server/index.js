...
// packages/reputation-service/dist/index.js
var index_exports = {};
...
init_ui_dialog();
var ReputationSettings = class _ReputationSettings {
  constructor({
  ...
  static fromArgs(options) {
    return new _ReputationSettings({
...
var ReputationClient = class {
  constructor(settings) {
    this.BASE_AUTH_URL = "https://rep.checkpoint.com/rep-auth/service/v1.0/request";
    this.BASE_REP_URL = "https://rep.checkpoint.com";
   ...
   */
  async getReputation(serviceType, resource) {
    try {
      const isTokenExpired = (token) => {
        const expMatch = token.match(/exp=(\d+)/);
        if (!expMatch) return true;
        const expTime = parseInt(expMatch[1]) * 1e3;
        return Date.now() >= expTime;
      };
      if (!tokenCached || isTokenExpired(tokenCached)) {
        const tokenResponse = await import_axios.default.get(this.BASE_AUTH_URL, {
          headers: {
      ...
      const repResponse = await import_axios.default.post(repUrl, {
        request: [{
          resource,
          context: {
            api_type: "mcp"
...
function getReputationVerdict(risk, confidence) {
...
server.tool(
  ...
  async (args, extra) => {
    ...
    try {
      ...
      const result = await client.getReputation("url", args.url);
...
server.tool(
  ...
  async (args, extra) => {
    ...
    try {
      const settings = SessionContext.getSettings(serverModule, extra);
      const client = new ReputationClient(settings);
      const result = await client.getReputation("ip", args.ip);
      const verdict = getReputationVerdict(result.risk, result.confidence);
      return {
        content: [{
          type: "text",
...
server.tool(
  ...
  async (args, extra) => {
    ...
    try {
      const settings = SessionContext.getSettings(serverModule, extra);
      const client = new ReputationClient(settings);
      const result = await client.getReputation("file", args.hash);
      const verdict = getReputationVerdict(result.risk, result.confidence);
      return {
        content: [{
          type: "text",
