...
## Why MCP for Check Point WAF?

The Check Point WAF MCP server provides AI-powered access to your Check Point WAF environment. It enables natural language interactions for querying WAF status, managing policies, and automating security operations through any MCP-compatible AI assistant.

## Use with other MCPs for Best Results

While Check Point WAF MCP works well on its own, it is designed to be used alongside other Check Point MCP servers (found in this repo) for comprehensive security management across your entire Check Point infrastructure.

## Features

- **Publish and enforce** configuration changes with safety confirmations
- Execute any GraphQL query/mutation against Check Point WAF API
- View WAF configuration settings
- Full GraphQL API integration with Check Point WAF
...
This MCP server makes API calls to your Check Point WAF instance. Response times depend on your WAF deployment and network connectivity.
...
- `WAF_CLIENT_ID`: Check Point WAF client ID for authentication
- `WAF_ACCESS_KEY`: Check Point WAF access key for authentication

To obtain these credentials, log in to the [Check Point Infinity Portal](https://portal.checkpoint.com), navigate to **Check Point WAF > Settings > API Keys**, and create a new API key. The `WAF_REGION` setting must match the region of your Infinity Portal account.

### Optional Configuration

- `WAF_REGION`: Check Point WAF region (default: `EU`)
...
| `call_waf_api` | Execute any GraphQL query/mutation against Check Point WAF API |
...
| `waf_consultant` | Best-practice recommendations grounded in official Check Point WAF documentation |

### GraphQL API Access

The `call_waf_api` tool allows you to execute any GraphQL query or mutation against the Check Point WAF API. Example queries:
...
```bash
MCP_TRANSPORT_TYPE=http MCP_TRANSPORT_PORT=3000 npx @chkp/checkpoint-waf-mcp
# or
npx @chkp/checkpoint-waf-mcp --transport http --transport-port 3000
```
...
```bash
node FULL_PATH_TO_SERVER/packages/checkpoint-waf/dist/index.js --client-id "your-client-id" --access-key "your-access-key" --region "EU"
```
...
1. **Authentication keys and credentials are never shared with the model.** They are used only by the MCP server to authenticate with your Check Point WAF instance.  
