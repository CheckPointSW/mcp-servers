...
const { server, pkg } = createMcpServer(import.meta.url, {
    description: 'Check Point WAF MCP server for Check Point WAF management',
});
...
const serverModule = createServerModule(
    ...
    Settings,
    pkg,
    CheckPointWAFAPIManager
);

// Tool to execute GraphQL queries against Check Point WAF API
server.tool(
    'call_waf_api',
    `Execute a GraphQL query or mutation against the Check Point WAF API. 
...
server.tool(
    'waf_consultant',
    'Check Point WAF consultant. Provides best-practice recommendations and guidance based on WAF configuration. Uses official Check Point WAF documentation to explain settings and suggest next actions or tools.',
    {
