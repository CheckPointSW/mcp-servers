...
export const DateRangeSchema = z.object({
    from: z
        .string()
        .describe(
            'Start date in ISO 8601 format (e.g., "YYYY-MM-DDTHH:MM:SSZ")'
        ),
    to: z
        .string()
        .describe('End date in ISO 8601 format (e.g., "YYYY-MM-DDTHH:MM:SSZ")'),
});

/**
 * Schema for flexible list parameters - handles JSON strings, arrays, or single values
 */
export const FlexibleListSchema = z
...
export const AlertFiltersSchema = z.object({
    severities: FlexibleListSchema.describe(
        'Alert severity levels: low, medium, high, very_high'
    ),
    statuses: FlexibleListSchema.describe(
        'Alert status: open, acknowledged, closed'
    ),
    types: FlexibleListSchema.describe(
        'Alert types (e.g., phishing_email, ransomware, etc.)'
    ),
    from_created_date: z
        .string()
        .optional()
        .describe('Start date for alerts (YYYY-MM-DD)'),
    to_created_date: z
        .string()
        .optional()
        .describe('End date for alerts (YYYY-MM-DD)'),
    limit: z
        .number()
        .min(10)
        .default(10)
        .describe('Number of alerts to return (minimum 10)'),
...
export const AssetFiltersSchema = z.object({
    ...
    created_from: z
        .string()
        .optional()
        .describe('Show assets created after this date (YYYY-MM-DD)'),
    ...
    fetch_technologies: z
        .boolean()
        .optional()
        .describe('Include technology stack information'),
});
...
export const IOCEnrichmentSchema = z.object({
    iocs: z
        .union([z.string(), z.array(z.string())])
        .describe(
            'Single IOC or list of IOCs (IPs, domains, URLs, file hashes)'
        ),
});
...
export const CVESearchSchema = z.object({
    ...
    cvss_min: z
        ...
        .optional()
        .describe('Minimum CVSS score (0.0-10.0)'),
    modified_days_back: z
        .number()
        .default(365)
        .describe('Search CVEs modified in last N days'),
    page_size: z
        .number()
        .min(1)
        .max(100)
...
export const CredentialExposureSchema = z.object({
    inputs: z
        .union([z.string(), z.array(z.string())])
        .describe('Domain(s) or email(s) to check'),
    mask_password: z
        .boolean()
        .default(true)
        .describe('Mask passwords in results'),
});
...
export const ThreatNewsFiltersSchema = z.object({
    ...
    filter_mode: z
        .string()
        .default('or')
        .describe('Logical operator: "or" or "and"'),
...
export const ThreatActorFiltersSchema = z.object({
    ...
    sectors: FlexibleListSchema.describe('Industry sectors filter'),
    filter_mode: FilterModeSchema.default('or').describe(
