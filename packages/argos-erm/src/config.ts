...
export class Settings {
    public argosHost = '';
    public argosApiKey = '';
    public argosCustomerId = '';

    constructor({
        argosHost = process.env.ARGOS_HOST || process.env.ARGOS_SERVER_URL,
        argosApiKey = process.env.ARGOS_API_KEY ||
            process.env.ARGOS_INTEGRATION_TOKEN,
    ...
    validate(): boolean {
        if (!this.argosHost) {
            throw new Error(
                'Argos host is required (via --argos-host or ARGOS_HOST/ARGOS_SERVER_URL env var)'
            );
        }
        if (!this.argosApiKey) {
            throw new Error(
                'Argos API key is required (via --argos-api-key or ARGOS_API_KEY/ARGOS_INTEGRATION_TOKEN env var)'
            );
        }
        // CUSTOMER_ID is now optional — multi-tenant mode when absent
        return true;
    }
